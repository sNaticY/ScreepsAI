...
interface Memory {
    purifier: PurifierMemory;
    sectors: { [name: string]: SectorMemory };
    creeps: { [name: string]: CreepMemory };
    flags: { [name: string]: FlagMemory };
    rooms: { [name: string]: RoomMemory };
    spawns: { [name: string]: SpawnMemory };
    tasks: { [name: string]: TaskMemory };
    forceClear: boolean;
}

interface PurifierMemory {
    unmanaged:{ [name: string]: number};
    sectorIds: string[];
}

type PROVINCESTATE_DEVELOPMENT = "DEVELOPMENT";
type PROVINCESTATE_TECHNOLOGY = "TECHNOLOGY";
type PROVINCESTATE_HOARDING = "HOARDING";
type PROVINCESTATE_SUPPLY = "SUPPLY";
type PROVINCESTATE_MILITARY = "MILITARY";

type ProvinceState = PROVINCESTATE_DEVELOPMENT|PROVINCESTATE_TECHNOLOGY|
                        PROVINCESTATE_HOARDING|PROVINCESTATE_SUPPLY|
                        PROVINCESTATE_MILITARY;

interface SectorMemory {
    name: string;
    roomNames: string[];
    state: ProvinceState;
    taskIds: { [name:string]: {[name:string]: {[name:string]: string}}}; //key0: roomId //key1:SourceId //Key2:taskType
    capitalRoomName: string;
}

type ROOMPLAN_MAJOR = "ROOMPLAN_MAJOR";
type ROOMPLAN_MINOR = "ROOMPLAN_MINOR";
type ROOMPLAN_JUNIOR = "ROOMPLAN_JUNIOR";
type ROOMPLAN_UNMANAGED = "ROOMPLAN_UNMANAGED";

type RoomPlan = ROOMPLAN_MAJOR|ROOMPLAN_MINOR|ROOMPLAN_JUNIOR|ROOMPLAN_UNMANAGED;

interface StructurePlan {
    [STRUCTURE_ROAD]: RoomPosition[];
    [STRUCTURE_CONTAINER]: RoomPosition[];
    [STRUCTURE_EXTENSION]: RoomPosition[];
    [STRUCTURE_TOWER]: RoomPosition[];
    [STRUCTURE_STORAGE]: RoomPosition[];
}

type ROOMSTATE_EMERGENCY = "ROOMSTATE_EMERGENCY";
type ROOMSTATE_CORE = "ROOMSTATE_CORE";
type ROOMSTATE_EXPANSION = "ROOMSTATE_EXPANSION";
type ROOMSTATE_UPGRADE = "ROOMSTATE_UPGRADE";
type ROOMSTATE_ARSENAL = "ROOMSTATE_ARSENAL";
type ROOMSTATE_BUILDING = "ROOMSTATE_BUILDING";
type ROOMSTATE_RESERVED = "ROOMSTATE_RESERVED";
type ROOMSTATE_REMOTEMINING = "ROOMSTATE_REMOTEMINING";


type RoomState = ROOMSTATE_EMERGENCY|ROOMSTATE_CORE|ROOMSTATE_EXPANSION|
                    ROOMSTATE_UPGRADE|ROOMSTATE_ARSENAL|ROOMSTATE_BUILDING|
                    ROOMSTATE_RESERVED|ROOMSTATE_REMOTEMINING;
...
interface RoomMemory {
    ...
    middlePos: RoomPosition;
    origin1Pos: RoomPosition;
    origin2Pos: RoomPosition;
    
    curSpawnTick: number;
    structurePlan: StructurePlan;
    state: RoomState;
    spawning: {[taskType: string]: SpawnPlan[]};

    econemyLevel: number;
}

type ROLE_BOOTSTRAPER = "BOOTSTRAPER";
type ROLE_HARVESTER = "HARVESTER";
type ROLE_CARRIER = "CARRIER";
type ROLE_UPGRADER = "UPGRADER";
type ROLE_BUILDER = "BUILDER";

type RoleType = ROLE_BOOTSTRAPER|ROLE_HARVESTER|ROLE_CARRIER|ROLE_UPGRADER|ROLE_BUILDER;
...
type TASK_BOOTSTRAP = "TASK_BOOTSTRAP";
type TASK_HARVEST = "TASK_HARVEST";
type TASK_CARRY = "TASK_CARRY";
type TASK_UPGRADE = "TASK_UPGRADE";
type TASK_BUILD = "TASK_BUILD";
type TASK_RESERVE = "TASK_RESERVE";
type TASK_CLAIM = "TASK_CLAIM";

type TaskType = TASK_BOOTSTRAP|TASK_HARVEST|TASK_CARRY|TASK_UPGRADE|TASK_RESERVE|
                TASK_CLAIM|TASK_BUILD;

interface TaskMemory {
    id: string;
    type: TaskType;
    workSource: string;
    homePos: RoomPosition;
    workPos: RoomPosition;
    pathLength: number;
    targetWorkPoint: number;
    curWorkPoint: number;
    planWorkPoint: number;
    workingCreepIds: string[];
}
