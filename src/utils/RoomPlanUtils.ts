...
export class RoomPlanUtils {
    ...
    public static FindEmptyCubeAreaAround(originPos: RoomPosition, room: Room, halfLength: number, minDistance: number, filter: PosFilter|null): RoomPosition | null {
        for (let range = halfLength + minDistance; range < 40; range++) {
            for (let i = range * -1; i < range; i++) {
                ...
                if (pos1 && RoomPlanUtils.CheckIfCubeEmpty(pos1, room, halfLength, filter)) { return pos1; }
                const pos2 = room.getPositionAt(originPos.x + i, originPos.y + range);
                if (pos2 && RoomPlanUtils.CheckIfCubeEmpty(pos2, room, halfLength, filter)) { return pos2; }
                const pos3 = room.getPositionAt(originPos.x - range, originPos.y + i);
                if (pos3 && RoomPlanUtils.CheckIfCubeEmpty(pos3, room, halfLength, filter)) { return pos3; }
                const pos4 = room.getPositionAt(originPos.x + range, originPos.y + i);
                if (pos4 && RoomPlanUtils.CheckIfCubeEmpty(pos4, room, halfLength, filter)) { return pos4; }
            }
        }
        return null;
    }
    ...
    public static FindRoadPositions(originPos: RoomPosition, room: Room): RoomPosition[] {
        ...
        positions.push(...RoomPlanUtils.FindRhumbus(originPos, room, 2, null));
        for (let i = 0; i < 3; i++) {
        ...
        if (pos5) {
            positions.push(...RoomPlanUtils.FindRhumbus(pos5, room, 1, null));
        }
        const pos6 = room.getPositionAt(originPos.x + 4, originPos.y);
        if (pos6) {
            positions.push(...RoomPlanUtils.FindRhumbus(pos6, room, 1, null));
        }
        const pos7 = room.getPositionAt(originPos.x, originPos.y - 4);
        if (pos7) {
            positions.push(...RoomPlanUtils.FindRhumbus(pos7, room, 1, null));
        }
        const pos8 = room.getPositionAt(originPos.x, originPos.y + 4);
        if (pos8) {
            positions.push(...RoomPlanUtils.FindRhumbus(pos8, room, 1, null));
        }
    ...
    public static FindExtensionPositions(originPos: RoomPosition, room: Room): RoomPosition[] {
        const positions: RoomPosition[] = [];
        const pos2s = RoomPlanUtils.FindCube(originPos, room, 2, (pos, i) => {
            return i !== -2 && i !== 0;
        });
        const pos3s = RoomPlanUtils.FindCube(originPos, room, 3, (pos, i) => {
            return i !== -3 && i !== 0;
        });
        const pos4s = RoomPlanUtils.FindCube(originPos, room, 3, (pos, i) => {
            return i !== -4 && i !== -1 && i !== -0 && i !== 1;
        });
        const pos5s = RoomPlanUtils.FindCube(originPos, room, 3, (pos, i) => {
